...
readFile(path.join(options.inputDir, 'analysis.json'))
  .then((jsonString: Buffer | string): Pick<Output, 'status' | 'comments'> => JSON.parse(jsonString.toString()))
  .then((output: Pick<Output, 'status' | 'comments'> ): void => {
    logger.log(`=> Got ${output.status} with ${output.comments.length} comments`)
    const spawned = childProcess.spawnSync(
      'ruby',
      [
        'bin/markdownify.rb',
        path.resolve(options.inputDir),
        path.resolve(path.join(__dirname, '..', '..', 'exercism-website-copy', 'automated-comments'))
      ],
      {
        stdio: 'pipe',
        cwd: process.cwd(),
