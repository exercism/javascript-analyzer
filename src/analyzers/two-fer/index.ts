...
export class TwoFerAnalyzer extends BaseAnalyzer {
  ...
  public async execute(): Promise<void> {
    ...
    // We often see solutions where there are correct solutions which don't use the default
  ...
  private checkSignature() {
    ...
    if (firstParameter.type === AST_NODE_TYPES.RestElement) {
      ...
      this.disapprove(
        UNEXPECTED_SPLAT_ARGS({
          splat_arg_name: splatArgName,
          parameter_type: splatArgType
        })
      );
    }
  }
  ...
  private checkForSolutionWithoutStringTemplate() {
    const [expression] = extractAll<BinaryExpression>(
      this.mainMethod!,
  ...
  private checkForSolutionWithFalsyDefault() {
    ...
    // `One for ${name || 'you'}, one for me.`
    //
    const expression = extractFirst<LogicalExpression>(
      this.mainMethod!,
    ...
    const conditionalExpression = extractFirst<ConditionalExpression>(
      this.mainMethod!,
  ...
  private checkForConditionalOnDefaultArgument() {
    const conditionalExpressions = extractAll<ConditionalExpression>(
      this.mainMethod!,
      AST_NODE_TYPES.ConditionalExpression
    );
    const ifStatements = extractAll<IfStatement>(
      this.mainMethod!,
    ...
    if (ifStatement) {
      ...
      if (
        ...
        isIdentifier(test.argument, defaultParameterName)
      ) {
        this.disapprove(
      ...
      if (
        ...
        (isIdentifier(test.left, defaultParameterName) ||
          isIdentifier(test.right, defaultParameterName))
      ) {
        this.disapprove(
          OPTIMISE_DEFAULT_VALUE({ parameter: defaultParameterName })
        );
    ...
    // !name ? `you` : 'name'
    if (
      (isIdentifier(consequent) &&
        (isLiteral(alternate, "you") || isTemplateLiteral(alternate))) ||
      (isIdentifier(alternate) &&
        (isLiteral(consequent, "you") || isTemplateLiteral(consequent)))
    ) {
      this.disapprove(
        OPTIMISE_DEFAULT_VALUE({ parameter: defaultParameterName })
      );
    ...
    if (
      (isTemplateLiteral(consequent) &&
        (isLiteral(alternate, "One for you, one for me.") ||
          isTemplateLiteral(alternate))) ||
      (isTemplateLiteral(alternate) &&
        (isLiteral(consequent, "One for you, one for me.") ||
          isTemplateLiteral(consequent)))
    ) {
      this.disapprove(
        OPTIMISE_DEFAULT_VALUE({ parameter: defaultParameterName })
  ...
  private hasThreeComponentsInTemplateLiteral() {
    const template = extractFirst<TemplateLiteral>(
      this.mainMethod!,
      AST_NODE_TYPES.TemplateLiteral
    );
    return (
