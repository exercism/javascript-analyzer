...
export function extractExport(program: Program, ...args: string[]): [ExportDeclarationNode, ExportedNode] {
  ...
  const type = name ? AST_NODE_TYPES.ExportNamedDeclaration : AST_NODE_TYPES.ExportDefaultDeclaration
          ...
          if (type === node.type) {
            ...
            if (node.declaration === null) {
              const specifier = node
                .specifiers
                .filter((specifier): boolean => specifier.type === AST_NODE_TYPES.ExportSpecifier)
                .find((specifier): boolean => specifier.local.name === name)
              ...
              switch(node.declaration.type) {
                ...
                case AST_NODE_TYPES.VariableDeclaration: {
                  const declaration = node
                    .declaration
                    .declarations
                    .find((d): boolean => d.id.type === AST_NODE_TYPES.Identifier && d.id.name === name)
                ...
                if (property.type === AST_NODE_TYPES.Identifier && property.name === 'exports') {
                  ...
                  switch (right.type) {
                    case AST_NODE_TYPES.ObjectExpression: {
                      const exportProperty = right.properties.find((property): boolean =>
