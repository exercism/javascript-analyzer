...
export function isMemberExpression<O extends string, P extends string | number>(node: Node, object: O, property: P): node is SpecificObjectProperty<O, P>
...
export function isMemberExpression<O extends string, P extends string | number>(node: Node, object?: O, property?: P): node is MemberExpression {
  return node.type === AST_NODE_TYPES.MemberExpression
    && (typeof object === 'undefined' || isIdentifier<O>(node.object, object))
    && (typeof property === 'undefined' || isIdentifier<string>(node.property, property as string) || isLiteral(node.property, property))
}
