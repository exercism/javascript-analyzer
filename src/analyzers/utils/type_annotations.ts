...
function annotate(typeNode?: TypeNode, fallback: string = 'any'): string {
  ...
  switch(typeNode.type) {
    ...
    case AST_NODE_TYPES.TSConstructorType:
      const ctP = typeNode.params.map(parameter => `${parameterName(parameter)}: ${parameterType(parameter, fallback)}`).join(', ')
    ...
    case AST_NODE_TYPES.TSFunctionType:
      const ftP = typeNode.params.map(parameter => `${parameterName(parameter)}: ${parameterType(parameter, fallback)}`).join(', ')
      const ftR = annotateType(typeNode.returnType, fallback)
      return `(${ftP}) => ${ftR}`
    case AST_NODE_TYPES.TSIntersectionType:
      return typeNode.types.map(innerTypeNode => annotate(innerTypeNode, fallback)).join(' & ')
    ...
    case AST_NODE_TYPES.TSTupleType:
      return `[${typeNode.elementTypes.map(innerTypeNode => annotate(innerTypeNode, fallback)).join(', ')}]`
